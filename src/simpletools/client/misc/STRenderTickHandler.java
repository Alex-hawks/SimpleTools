...
public class STRenderTickHandler implements ITickHandler
{
	...
	public void tickEnd(EnumSet<TickType> type, Object... tickData)
	{
		...
		if (player != null && !(Loader.isModLoaded("" /*NEI*/) || Minecraft.getMinecraft().currentScreen != null)) 
		{
			...
			for (int i = 0; i < 9; i++)
			{
				if (player.inventory.getStackInSlot(i) != null && player.inventory.getStackInSlot(i).getItem() instanceof IAssembledTool)
				{
					ItemStack tool = player.inventory.getStackInSlot(i);
					String display = ((IAssembledTool) player.inventory.getStackInSlot(i).getItem()).getStoredForDisplay(tool);
