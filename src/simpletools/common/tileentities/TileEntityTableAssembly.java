...
public class TileEntityTableAssembly extends TileEntityAdvanced implements IRedstoneProvider, IPacketReceiver, ISidedInventory
{
	...
	{
		if (this.inventory[3] == null && this.canAssemble() && !assembling)
		{
			this.startAssemble();
			this.assembling = true;
		}
		else if (this.inventory[3] == null && this.canAssemble() && assembling)
		{
			this.finishAssemble();
	...
	public boolean canAssemble()
	{
		...
		if (slot0 != null && slot1 != null && slot2 != null)
		{
			if (slot0.getItem() instanceof IAttachment && slot1.getItem() instanceof ICore)
			{
				...
				{
					if (coreTemp.getCoreFinerType(slot1).equals("electric") && slot2.getItem() instanceof IItemElectric && ((IItemElectric)slot2.getItem()).getProvideRequest(slot2).getWatts() > 0)
					{
						return true;
					}
					else if (coreTemp.getCoreFinerType(slot1).equals("plasma") && slot2.getItem() instanceof IItemElectric && ((IItemElectric)slot2.getItem()).getProvideRequest(slot2).getWatts() > 0 )
					{
						return true;
					}
