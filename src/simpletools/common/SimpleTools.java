...
//1		tableRefuel				= 
		...
		assembledToolElectric	= new ItemAssembledToolElectric(config.getItem("Electric_Assembled_Tool", FIRST_TOOL_ID).getInt(), "MechElectricAssembledTool");
		assembledToolPlasma		= new ItemAssembledToolPlasma(config.getItem("Plasma_Assembled_Tool", FIRST_TOOL_ID + 1).getInt(), "PlasmaAssembledTool");
//2		assembledToolFuel
		
		coreMechElectric 		= new ItemCoreMotor(config.getItem("Electric_Motor_Core", FIRST_CORE_ID).getInt(), "CoreMechElectric");
		corePlasma				= new ItemCorePlasma(config.getItem("Plasma_Core", FIRST_CORE_ID + 1).getInt(), "CorePlasma");
//2		coreMechFuel			=
		
		attachmentToolMotor		= new ItemAttachmentToolMotor(config.getItem("Motor_Tool_Attachment", FIRST_BIT_ID).getInt(), "MotorTool");
		attachmentToolPlasma	= new ItemAttachmentPlasma(config.getItem("Plasma_Tool_Attachment", FIRST_BIT_ID + 1).getInt(), "PlasmaTool");
